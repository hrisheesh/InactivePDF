...
const state = { token:'', config:null, draft:null, route:'overview', status:null, metrics:null, history:[], busy:false, profiles:{}, assets:[], imageUrls:new Map(), wm:null, wmSaved:null, wmName:'', wmDirty:false, analyticsSource:'' };
const sections = [
...
function applyLive(snapshot){
 ...
 state.status=snapshot.status;state.metrics=snapshot.metrics;state.analytics=snapshot.analytics;state.liveJobs=snapshot.jobs;state.liveLogs=snapshot.logs;state.liveDeadLetters=snapshot.deadLetters;
 ...
 if(state.route==='overview')renderOverview();if(state.route==='security')renderSecurity();
...
function renderOverview(){
 ...
 <article class="panel format-panel"><div class="panel-head"><div><h2>Time by document format</h2><p>Measured worker execution · excludes queue and upload time.</p></div><span class="pill">${a.formats.length} formats observed</span></div><div class="table-wrap"><table><thead><tr><th>Format</th><th>Attempts</th><th>Successful</th><th>Average</th><th>Min</th><th>Max</th></tr></thead><tbody>${a.formats.length?a.formats.map(f=>`<tr><td><span class="format-badge">${escapeHtml(f.format.toUpperCase()||'UNKNOWN')}</span></td><td>${f.attempts}</td><td>${f.timing.count}</td><td>${ms(f.timing.averageMs)}</td><td>${ms(f.timing.minMs)}</td><td>${ms(f.timing.maxMs)}</td></tr>`).join(''):'<tr><td colspan="6" class="empty">No timing observations yet.</td></tr>'}</tbody></table></div></article>
 <article class="panel"><div class="panel-head"><div><h2>Intake & output</h2><p>Where conversion work comes from.</p></div></div><div class="panel-body">${a.sources.map(source=>stat(source.source,source.attempts+' attempts · '+source.succeeded+' succeeded')).join('')||'<p>No sources observed yet.</p>'}${stat('Input volume',bytes(a.inputBytes))}${stat('Successful output',bytes(a.outputBytes))}${stat('All executions · average',ms(a.executionTiming.averageMs))}${stat('Interrupted by restart',a.interrupted)}</div></article>
...
const choices={DefaultProfile:['archive','compact','compatibility'],DefaultOperation:['ConvertFile','ConvertFiles','ConvertAndMerge','CreateTextPdf'],PdfVersion:[14,15,16,17,20],LargeMarkupPolicy:['visual-layout']};
...
function renderSettingPreview(){
 ...
 if(route==='Performance'){const p=draft.Performance;content=stat('Profile',p.Profile)+stat('Shared worker ceiling',p.MaximumParallelWorkers)+stat('Estimated admission budget',bytes(p.MemoryBudgetBytes))+stat('Fairness threshold',p.AgingSeconds+' seconds')+'<p>The scheduler shares capacity across API and watch-folder work. Engine limits and resource limits still apply. PDF quality settings are preserved.</p>';}
...
$('refreshButton').onclick=async()=>{await refresh();try{if(state.route==='jobs')await loadDeadLetters();else if(!['overview','watermarks','security'].includes(state.route)&&!changes().length){await loadConfig();renderSettings();}}catch(error){notice(error.message,true);}};
...
globalSearchHost.innerHTML='<label class="global-search-input"><span aria-hidden="true">⌕</span><input id="globalSearch" type="search" maxlength="128" autocomplete="off" placeholder="Search settings, jobs, logs…" aria-label="Search all settings, jobs, and logs" aria-controls="globalResults" aria-expanded="false"><kbd>⌘ K</kbd></label><div id="globalResults" class="global-results" hidden></div>';
...
async function searchEverywhere(){
 ...
 try{
  ...
  const remote=[...response.jobs.map(j=>({category:'Job',title:j.jobId,detail:j.state+' · '+j.correlationId,route:'jobs',job:j.jobId})),
