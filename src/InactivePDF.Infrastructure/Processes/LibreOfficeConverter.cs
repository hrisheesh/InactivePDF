...
{
    public async Task<OfficeConversionResult> ConvertAsync(string inputPath, string outputPath, CancellationToken cancellationToken = default)
    {
        ...
        LibreOfficeProfileProvisioner.Create(profileDirectory);
        var started = Stopwatch.StartNew();
        try
        {
            var startInfo = BuildStartInfo(input, workingDirectory, profileDirectory);
    ...
    private ProcessStartInfo BuildStartInfo(string input, string outputDirectory, string profile)
    {
        var arguments = $"--headless --invisible --nodefault --nologo --nolockcheck --norestore --nofirststartwizard -env:UserInstallation={ToFileUri(profile)} --convert-to pdf --outdir {Quote(outputDirectory)} {Quote(input)}";
